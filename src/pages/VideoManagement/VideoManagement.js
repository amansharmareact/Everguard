...
import { VideoDataValidator } from "../../utils/validators";
...
const OfferManagement = ({ history, setUsers, userData }) => {
  ...
  const deleteVideo = async (e) => {
    ...
    if (window.confirm("Are you sure you want to delete this video?")) {
      try {
        const { data } = await axios.delete(`/admin/removeVideo?video_id=${e.categoryId}`);
        getVideoList(page + 1, rowsPerPage, searchValue);
        // getVideoList()
        toast.success(data.message, {
          position: toast.POSITION.TOP_RIGHT,
  ...
  const getVideoList = async (page = 1, rowsPerPage = 15, searchValue = "", startDate = "", endDate = "", dateFilter = "") => {
    setIsLoading(true);
    try {
      const { data } = await axios.get(`/admin/getVideo?page=${page}&pageSize=${rowsPerPage}&searchTerm=${searchValue}`);
  ...
  const handleSubmit = async (values) => {
    ...
    let addUrl = `/admin/createVideo`;
    let VideoData;

    VideoData = {
      title: values.title,
      thumbnail: values.file[0],
      link: values.link,
      description: values.description,
    };

    try {
      const { data } = await axios.post(addUrl, VideoData);
      console.log(data);
      toast.success(`${data.message}`, {
        position: toast.POSITION.TOP_RIGHT,
      });
  ...
  const handleEditBlog = async (values) => {
    ...
    let updateUrl = `/admin/updateVideo`;
    let VideoData;
    ...
    try {
      const { data } = await axios.patch(updateUrl, VideoData);
      console.log(data);
      toast.success(`${data.message}`, {
        position: toast.POSITION.TOP_RIGHT,
      });
  ...
  const statusSwitch = async (e, id) => {
    try {
      ...
      const { data } = await axios.post("/admin/blockUnblockVideo", {
        video_id: id,
      ...
      <div>
        <DashboardContainer>
          <DashboardWrapper>
            <DashboardHeading>
              <MenuAndBack>
                <DashHeading>Video Management</DashHeading>
              </MenuAndBack>
              <div style={{ display: "flex", gap: "1rem" }}>
                ...
                <Tooltip title={<span style={{ color: "white", fontSize: "16px" }}>Create Video </span>} arrow>
                  <IconButton
                    className=""
                    style={{
                        ...
                        <TableRow key={category._id}>
                          ...
                          <TableCell className={classes.textMiddle}>
                            {!get(category, "thumbnail", "") ? (
                              ...
                              <img src={get(category, "thumbnail", "")} alt="image" style={{ width: "70px", height: "50px" }} />
                            )}
                          </TableCell>
      ...
      <Modal
        ...
        title={
          <div>
            ...
            >
              {!VideoData._id ? `Add Video` : `Edit Video`}
            </div>
            <div className="">
              <SlClose
                style={{
