...
const OfferManagement = ({ history, setUsers, userData }) => {
  ...
  // For Pagination
  const [page, setPage] = React.useState(!state ? 0 : state[0].page);
  const [rowsPerPage, setRowsPerPage] = React.useState(!state ? 15 : state[0].rowsPerPage);
  ...
  const getFilteredMerchantList = async () => {
    console.log("fire");
    try {
      const { data } = await axios.get(`/admin/listmerchants?approvalStatus=approved&from=${startDate}&to=${endDate}`);
      setTableData(data.data);
      setSearchedData(data.data);
      // if (data.data.length === 0) {
      //   toast.error("No Data Found", {
      ...
      <div>
        <DashboardContainer>
          <DashboardWrapper>
            <DashboardHeading>
              <MenuAndBack>
                <DashHeading>Approved Merchants List</DashHeading>
              </MenuAndBack>
                ...
                <Table stickyHeader>
                  <TableHead>
                    <TableRow>
                      ...
                      <TableCell className={classes.tablePadding}>View Products</TableCell>
                      <TableCell className={classes.tablePadding}>View Coupons</TableCell>
                      ...
                      <TableRow key={category._id}>
                        ...
                        <TableCell className={classes.textMiddle}>
                          <div> <Button
                              ...
                              onClick={() => {
                                history.push({
                                  pathname: `/adminPanel/merchant_details_approved/view_products/${category._id}`,
                        ...
                        <TableCell className={classes.textMiddle}>
                          <div> <Button
                              ...
                              onClick={() => {
                                history.push({
                                  pathname: `/adminPanel/merchant_details_approved/view_coupons/${category._id}`,
                                   state: [{ page: page, rowsPerPage: rowsPerPage, location: history.location.pathname }],
                                });
                              }}
                            >
                        ...
                        <TableCell className={classes.tableFlex}>
                          <div>
                            <Button
                              ...
                              onClick={() => {
                                history.push({
                                  pathname: `/adminPanel/merchant_details_approved/${category._id}`,
                                  state: [{ page: page, rowsPerPage: rowsPerPage, location: history.location.pathname }],
                                });
                              }}
                            >
