...
const OfferManagement = ({ history, setUsers, userData }) => {
  ...
  const getMerchantList = async () => {
    setIsLoading(true);
    try {
      const { data } = await axios.get(`/admin/listmerchants?approvalStatus=pending`);
      setTableData(data.data);
      setSearchedData(data.data);
  ...
  const getFilteredMerchantList = async () => {
    console.log("fire");
    try {
      const { data } = await axios.get(`/admin/listmerchants?approvalStatus=pending&from=${startDate}&to=${endDate}`);
      setTableData(data.data);
      setSearchedData(data.data);
      console.log(data);
      // if (data.data.length === 0) {
      ...
      <div>
        <DashboardContainer>
          <DashboardWrapper>
            <DashboardHeading>
              <MenuAndBack>
                <DashHeading>Pending Merchants List</DashHeading>
              </MenuAndBack>
                        ...
                        <TableCell className={classes.tableFlex}>
                          ...
                          <div>
                            <Button
                              ...
                              onClick={() => {
                                history.push({
                                  pathname: `/adminPanel/merchant_details_pending/${category._id}`,
                          ...
                          {/* <div>
                            <Button
                              ...
                              className={classes.Marginbutton}
                              onClick={() => {
                                disapproveMerchant(category._id);
                              }}
                            >
                              Disapprove
                            </Button>
