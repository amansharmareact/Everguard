...
import { PreferredSubscriptionDataValidator } from "../../utils/validators";
...
const OfferManagement = ({ history, setUsers, userData }) => {
  ...
  const getsubscriptionPlanList = async () => {
    setIsLoading(true);
    try {
      const { data } = await axios.get(`/admin/get_preferred_membership_content`);
      if (data.data) {
  ...
  const DeleteSubscriptionPlan = async (id) => {
    if (window.confirm("Are you sure you want to delete this Preferred Subscription ?")) {
      try {
        const { data } = await axios.delete(`/admin/remove_preferred_membership_content?memberContent_id=${id}`);
  ...
  const handleSubmit = async (values) => {
    ...
    if (!values._id) {
      ...
      let addUrl = `/admin/create_preferred_membership_content`;
  ...
  const handleEditSubcription = async (values) => {
    let updateUrl = `/admin/update_preferred_membership_content`;
    let subscriptionData;
      ...
      <div>
        <DashboardContainer>
          <DashboardWrapper>
            <DashboardHeading>
              <MenuAndBack>
                <DashHeading>Preferred Subscription</DashHeading>
              </MenuAndBack>
              ...
              {tableData.length >= 1 ? (
                ...
                <div style={{ display: "flex", gap: "1rem" }}>
                  ...
                  <Tooltip
                    title={<span style={{ color: "white", fontSize: "16px" }}>Create Preferred Subscription </span>}
                    arrow
                  >
                    <IconButton
                      className=""
        ...
        content={
          <>
            <Formik
              ...
              validate={PreferredSubscriptionDataValidator}
              validateOnChange
              onSubmit={(values) => {
                if (values._id) {
