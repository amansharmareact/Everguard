  ...
  const getBlogList = async (page = 1, rowsPerPage = 15, searchValue = "") => {
    ...
    try {
      const { data } = await axios.get(
        `/admin/getPreferredMember?page=${page}&pageSize=${rowsPerPage}&searchTerm=${searchValue}`
      );
      ...
      <div>
        <DashboardContainer>
          <DashboardWrapper>
            <DashboardHeading>
              <MenuAndBack>
                <DashHeading>Preferred Enquiry Listing</DashHeading>
              </MenuAndBack>
                ...
                <Table stickyHeader>
                  <TableHead>
                    <TableRow>
                      ...
                      <TableCell className={classes.tablePadding}>Business Name</TableCell>
                      <TableCell className={classes.tablePadding}>Industry Type</TableCell>
                      <TableCell className={classes.tablePadding}>Image</TableCell>
                  ...
                  <TableBody>
                    {
                      ...
                      recordsAfterPagingAndSorting().map((category, index) => (
                        <TableRow key={category._id}>
                          ...
                          <TableCell className={classes.textMiddle} style={{ textTransform: "capitalize" }}>
                            {get(category, "business_name", "")}
                          </TableCell>
                          <TableCell className={classes.textMiddle} style={{ textTransform: "capitalize" }}>
                            {get(category, "industry_type", "")}
                          </TableCell>
                          <TableCell className={classes.textMiddle}>
                            {!get(category, "logo", "") ? (
                              "N/A"
                            ) : (
                              <img src={get(category, "logo", "")} alt="image" style={{ width: "30px", height: "30px" }} />
                            )}
                          </TableCell>
                          <TableCell className={classes.textMiddle}>
                            {
                              <Tooltip title={LinkText === category._id ? "Click to Hide" : "Click to View"} arrow>
                                ...
                                >
                                  {!get(category, "video", "")?"N/A":get(category, "video", "")}
                                </div>
                              </Tooltip>
                            }
