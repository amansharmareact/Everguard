...
const OfferManagement = ({ history, setUsers, userData }) => {
  ...
  const getBlogList = async () => {
    setIsLoading(true);
    try {
      const { data } = await axios.get(`/admin/getPreferredContent`);
      ...
      <div>
        <DashboardContainer>
          <DashboardWrapper>
            <DashboardHeading>
              <MenuAndBack>
                <DashHeading>Preferred Management</DashHeading>
              </MenuAndBack>
              <div style={{ display: "flex", gap: "1rem" }}>
                ...
                <Tooltip title={<span style={{ color: "white", fontSize: "16px" }}>Create Preferred Content</span>} arrow>
                  <IconButton
                    className=""
                    style={{
