...
import { CategoryDataValidator } from "../../utils/validators";
...
const OfferManagement = ({ history, setUsers, userData }) => {
  ...
  const handleChangePage = (event, newPage) => {
    // setPage(newPage);
    getCategoryList(newPage + 1, rowsPerPage, searchValue);
  };
  ...
  const handleChangeRowsPerPage = (event) => {
    ...
    getCategoryList(1, event.target.value, searchValue);
  ...
  const getCategoryList = async (page = 1, rowsPerPage = 15, searchValue = "") => {
    setIsLoading(true);
    try {
      const { data } = await axios.get(`/admin/getCategory?page=${page}&pageSize=${rowsPerPage}&searchTerm=${searchValue}`);
  ...
  const DeleteCategory = async (id) => {
    if (window.confirm("Are you sure you want to delete this category?")) {
      try {
        const { data } = await axios.delete(`/admin/removeCategory?category_id=${id}`);
  ...
  const statusSwitch = async (e, category) => {
    ...
    try {
      const { data } = await axios.post(`/admin/blockUnblockCategory`, {
      ...
      getCategoryList(page + 1, rowsPerPage, searchValue);
    } catch (error) {
      console.log(error);
    }
    // } else {
  ...
  const handleSubmit = async (values) => {
    ...
    let addUrl = `/admin/createCategory`;
    let categoryData;
  ...
  const handleEditCategory = async (values) => {
    ...
    try {
      const { data } = await axios.patch(updateUrl, categoryData);
      console.log(data);
      toast.success(`${data.message}`, {
        position: toast.POSITION.TOP_RIGHT,
      });
  ...
  const SearchUser = myDeb((e) => {
    ...
    getCategoryList(page + 1, rowsPerPage, value.toLowerCase());
    // console.log(value)
  });
  return (
    <>
                  ...
                  <TableBody>
                    {recordsAfterPagingAndSorting().map((category, index) => (
                      <TableRow key={category._id}>
                        ...
                        <TableCell className={classes.textMiddle}>
                          <div style={{ textTransform: "capitalize" }}>{get(category, "name", "")}</div>
                        </TableCell>
      ...
      <Modal
        ...
        title={
          <div>
            ...
            >
              {!BannerData._id ? `Add Category` : `Edit Category`}
            </div>
            <div className="">
              <SlClose
                style={{
