...
const OfferManagement = ({ history, setUsers, userData }) => {
  ...
  const cancelSearch = () => {
    setSearched("");
    getStoreList(page + 1, rowsPerPage, searchValue);
    // getStoreList()
  };
  ...
  const deleteStore = async (e) => {
    ...
    if (window.confirm("Are you sure you want to delete this store?")) {
      try {
        const { data } = await axios.delete(`/admin/removeStore?store_id=${e.categoryId}`);
        getStoreList(page + 1, rowsPerPage, searchValue);
        // getStoreList()
        toast.success(data.message, {
          position: toast.POSITION.TOP_RIGHT,
  ...
  const getStoreList = async (page = 1, rowsPerPage = 15, searchValue = "", startDate = "", endDate = "", dateFilter = "") => {
    setIsLoading(true);
    try {
      const { data } = await axios.get(`/admin/getStore?page=${page}&pageSize=${rowsPerPage}&searchTerm=${searchValue}`);
  ...
  const SearchUser = myDeb((e) => {
    ...
    getStoreList(page + 1, rowsPerPage, value.toLowerCase());
    // console.log(value)
  });
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      const { data } = await axios.post(addUrl, StoreData);
      console.log(data);
      toast.success(`${data.message}`, {
        position: toast.POSITION.TOP_RIGHT,
      });
  ...
  const handleEditBlog = async (values) => {
    ...
    let updateUrl = `/admin/updateStore`;
    let StoreData;
    ...
    try {
      const { data } = await axios.patch(updateUrl, StoreData);
      console.log(data);
      toast.success(`${data.message}`, {
        position: toast.POSITION.TOP_RIGHT,
      });
  ...
  const statusSwitch = async (e, id) => {
    try {
      ...
      const { data } = await axios.post("/admin/blockUnblockStore", {
        store_id: id,
      ...
      <div>
        <DashboardContainer>
          <DashboardWrapper>
            <DashboardHeading>
              <MenuAndBack>
                <DashHeading>Top Stores Management</DashHeading>
              </MenuAndBack>
              <div style={{ display: "flex", gap: "1rem" }}>
                ...
                <Tooltip title={<span style={{ color: "white", fontSize: "16px" }}>Create Store </span>} arrow>
                  <IconButton
                    className=""
                    style={{
                          ...
                          <span
                            ...
                            onClick={() => {
                              ...
                              if (startDate === null && endDate === null) {
                                ...
                                setShowFilter(false);
                                // getFilteredUserList();
                                getStoreList(page+1,rowsPerPage,"")
                              }
                            }else{
                      ...
                      recordsAfterPagingAndSorting().map((category, index) => (
                        <TableRow key={category._id}>
                          ...
                          <TableCell className={classes.textMiddle}>
                            <div>{categoryListData?.filter((ele) => ele.value == category.category)[0]?.label}</div>
                          </TableCell>
                          ...
                          <TableCell className={classes.textMiddle}>
                            {!get(category, "icon", "") ? (
                              ...
                              <img src={get(category, "icon", "")} alt="image" style={{ width: "70px", height: "50px" }} />
                            )}
                          </TableCell>
                          {/* <TableCell className={classes.textMiddle}>
                                                            <div>
                                ...
                                <Tooltip title={"Edit"} arrow>
                                  <Button
                                    ...
                                    onClick={() => {
                                      setStoreData({
                                        name: category.name,
                                        category: categoryListData?.filter((ele) => ele.value == category.category)[0],
                                        file: [category.icon],
                                        link: category.link,
                              ...
                              <div>
                                <Tooltip title={"Delete"} arrow>
                                  <Button
                                    ...
                                    // className={classes.Marginbutton}
                                    onClick={() => {
                                      deleteStore({ categoryId: category._id });
                                    }}
                                  >
                                    <DeleteOutline />
                                  </Button>
      ...
      <Modal
        ...
        title={
          <div>
            ...
            >
              {!StoreData._id ? `Add Store` : `Edit Store`}
            </div>
            <div className="">
              <SlClose
                style={{
                  ...
                  <Form style={{ margin: "1rem 1rem 0 1rem" }}>
                    <div className="signup-cont">
                      <div className="row">
                        ...
                        <div className="col-md-12">
                          <label className={classes.offerLabel}>Category</label>
                          <Field name="category">
                            {({ field }) => (
                              <div className="pb-2 mt-1 ">
                                ...
                                />
                                {get(formikBag, "touched.category", false) && get(formikBag, "errors.category", null) && (
                                  <p
                                    style={{
                                      paddingTop: 5,
                                      fontSize: 13,
                                  ...
                                  >
                                    {get(formikBag, "errors.category", null)}
                                  </p>
                                )}
                              </div>
                            )}
