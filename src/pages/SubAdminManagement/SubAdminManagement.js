...
const [edit,setEdit]=useState(false)
  ...
  // For Pagination
  const [page, setPage] = React.useState(!state?0:state.page);
  const [rowsPerPage, setRowsPerPage] = React.useState(!state?15:state.rowsPerPage);
  ...
  const getSubAdminList = async () => {
    ...
    setIsLoading(true);
    try {
      const { data } = await axios.get(`/admin/subadminListing`);
      setTableData(data.data);
      setSearchedData(data.data);
  ...
  const DeleteSubAdmin = async (id) => {
    try {
      const { data } = await axios.post(`/admin/deleteSubadmin`, { _id: id });
      toast.success(`${data.message}`, {
        position: toast.POSITION.TOP_RIGHT,
      });
      getSubAdminList()
      ...
      <div>
        <DashboardContainer>
          <DashboardWrapper>
            <DashboardHeading>
              <MenuAndBack>
                <DashHeading>SubAdmin Management</DashHeading>
              </MenuAndBack>
              ...
              <div style={{ display: "flex", gap: "1rem" }}>
                ...
                <Tooltip title={<span style={{ color: "white", fontSize: "16px" }}>Create SubAdmin</span>} arrow>
                  <IconButton
                    className=""
                    style={{
                    ...
                    onClick={() => {
                      history.push({
                        pathname:`/adminPanel/subAdminAddEdit`,
                        // state: [{ page: page, rowsPerPage: rowsPerPage, location: history.location.pathname }],
                      })
                      // setOpenModal(true)
                ...
                <Table stickyHeader>
                  <TableHead>
                    <TableRow>
                      ...
                      <TableCell className={classes.tablePadding}>SubAdmin Id</TableCell>
                    ...
                    {recordsAfterPagingAndSorting().map((category, index) => (
                      <TableRow key={category._id}>
                        ...
                        <TableCell className={classes.textMiddle}>
                          <div>{get(category, "fullName", "")}</div>
                        </TableCell>
                        <TableCell className={classes.textMiddle} style={{ textTransform: "capitalize" }}>
                          <div>
                          <img src={get(category,"profilePhoto","N/A")} alt="image" style={{width:"30px",height:"30px"}}/>
                          </div>
                        </TableCell>
                        <TableCell className={classes.textMiddle}>
                            ...
                            <Tooltip title="Edit" arrow>
                              <Button
                                ...
                                onClick={() => {
                                  ...
                                  let cat=category;
                                  cat.page=page;
                                  cat.rowsPerPage=rowsPerPage;
                                  history.push({
                                    pathname:`/adminPanel/subAdminAddEdit/${category._id}`,
                                    state:cat
                                  })
                                }}
                              >
