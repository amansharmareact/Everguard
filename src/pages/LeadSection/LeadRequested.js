...
const OfferManagement = ({ history, setUsers, userData, }) => {
    ...
    const getPropertyList = async () => {
        try {
            const { data } = await axios.get(`/admin/get-leads-with-zipcode/${params?.id}`);
                                    ...
                                    {recordsAfterPagingAndSorting().map((category, index) => (
                                        <TableRow key={category._id}>
                                            ...
                                            <TableCell className={classes.textMiddle}>
                                                <div>
                                                {dateOfJoining(category?.date)}
                                                </div>
                                            </TableCell>
                                            <TableCell className={classes.textMiddle}>
                                                <div>
                                                {category?.requested_by?.role}
                                                </div>
                                            </TableCell>
                                            <TableCell className={classes.tableFlex}>
                                                <div style={{marginRight : '1rem'}}>
                                                {category?.requested_by?.first_name} {" "} {category?.requested_by?.last_name}
                                                </div>
                                                <Button 
                                                            variant="outlined"
                                                                aria-label="add"
                                                                ...
                                                                onClick={() => {
                                                                history.push(`/adminPanel/agent-approve/${category?.requested_by?._id}?agent=true`)
                                                            }}  
                                            ...
                                            <TableCell className={classes.textMiddle}>
                                                ...
                                                onClick={() => {
                                                    history.push(`/adminPanel/lead/assigned/${category?._id}`)
                                                  }}  
                                                >
                                                 View All
                                                </div>
                                            ...
                                            <TableCell className={classes.textMiddle}>
                                            <div>
                                            <a href={ `/webPanel/property/${category.property_id}`} className="term-link" target="_blank">
                                                    <Button 
