...
import { withRouter, useParams, useLocation, useSearchParams } from "react-router-dom";
...
const OfferManagement = ({ history, setUsers, userData }) => {
  ...
  //  const [trackId, setTrackId] = useState(new URLSearchParams(query).get('agent'))
  ...
  // const locationUrl = async (e) => {
  //     const values = await queryString.parse(location?.search)
  //     console.log(values);
  //     if(values.agent) {
  //         console.log("pass");
  //     } else {
  ...
  const para = new Proxy(new URLSearchParams(window.location.search), {
    get: (searchParams, prop) => searchParams.get(prop),
  });
  const [propertyId, setPropertyId] = useState(para.agent);
  ...
  return (
    <>
      <div>
        <DashboardContainer>
          <DashboardWrapper style={{ height: "auto" }}>
            <DashboardHeading>
              <MenuAndBack>
                <BackIcon>
                  <>
                    <HeadingButton
                      ...
                      onClick={() => {
                        propertyId ? history.push("/adminPanel/property") : history.push("/adminPanel/agent-approve");
                      }}
                    >
            ...
            <Paper className={classes.paperTableHeight} style={{ overflow: "hidden", height: "100%" }}>
              <>
                <RetaurantDetailsForm>
                  ...
                  <ServiceAreaText>Manage Service Area List</ServiceAreaText>
                  {tableData?.service_area?.map((qy, index) => (
                    <ServiceTable>
                      <ServiceTableRowHeader>
                      ...
                      {qy?.service_area?.map((svc, index) => (
                        <ServiceTableRow>
                          <ServiceTableColumn>{svc?.zip_code}</ServiceTableColumn>
                          <ServiceTableColumn>{svc?.area_percent}</ServiceTableColumn>
                          <ServiceTableColumn>{2}</ServiceTableColumn>
                          <ServiceTableColumn>
                            <AddAmountWrap>
                              <input
                                type="number"
                      ...
                      <TotalTableRow>
                        <TotalTableColumn>Total Amount</TotalTableColumn>
                        <TotalTableColumn></TotalTableColumn>
                        <TotalTableColumn></TotalTableColumn>
                        <TotalTableColumn>{qy?.total_amount}</TotalTableColumn>
                      </TotalTableRow>
                    </ServiceTable>
                  ))}
                </RetaurantDetailsForm>
