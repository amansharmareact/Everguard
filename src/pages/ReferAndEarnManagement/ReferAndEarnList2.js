...
const OfferManagement = ({ history, setUsers, userData }) => {
  ...
  const handleChangePage = (event, newPage) => {
    // setPage(newPage);
    getReferList(newPage + 1, rowsPerPage, searchValue, id);
  };
  ...
  const handleChangeRowsPerPage = (event) => {
    ...
    getReferList(1, event.target.value, searchValue, id);
  ...
  const cancelSearch = () => {
    setSearched("");
    getReferList(page + 1, rowsPerPage, searchValue, id);
    // getReferList()
  };
    ...
    if (e.categoryBlocked === true) {
      if (window.confirm("Are you sure you want to unblock this blog?")) {
        try {
          ...
          getReferList(page + 1, rowsPerPage, searchValue);
          // getReferList()
  ...
  const getReferList = async (page = 1, rowsPerPage = 15, searchValue = "", id = "") => {
    setIsLoading(true);
    try {
      const { data } = await axios.get(
        `/admin/get_referral_user_details_by_id?page=${page}&pageSize=${rowsPerPage}&searchTerm=${searchValue}&user_id=${id}`
      );
  ...
  const SearchUser = myDeb((e) => {
    ...
    getReferList(page + 1, rowsPerPage, value.toLowerCase(), id);
    // console.log(value)
  });
            ...
            <DashboardHeading>
              <MenuAndBack>
                <ArrowBackIosIcon
                  ...
                  onClick={() => history.push(`/adminPanel/referList`)}
                />
                <DashHeading>View List</DashHeading>
              </MenuAndBack>
              <div style={{ display: "flex", gap: "1rem" }}>
                ...
                {/* <Tooltip title={<span style={{ color: "white", fontSize: "16px" }}>Create Content </span>} arrow>
                  <IconButton
                    className=""
                    style={{
                          ...
                          <span
                            ...
                            onClick={() => {
                              ...
                              if (startDate === null && endDate === null) {
                                ...
                                setShowFilter(false);
                                // getFilteredUserList();
                                getReferList(page+1,rowsPerPage,"")
                              }
                            }else{
                  ...
                  <TableBody>
                    {
                      ...
                      recordsAfterPagingAndSorting().map((category, index) => (
                        <TableRow key={category._id}>
                          ...
                          <TableCell className={classes.textMiddle}>
                            <div>{get(category, "referral_code_count", "N/A")}</div>
                          </TableCell>
      ...
      <Modal
        ...
        title={
          <div>
            ...
            >
              {!ReferData._id ? `Add Content` : `Edit Content`}
            </div>
            <div className="">
              <SlClose
                style={{
