  ...
  const getVideoList = async (page = 1, rowsPerPage = 15, searchValue = "", startDate = "", endDate = "", dateFilter = "") => {
    ...
    try {
      const { data } = await axios.get(
        `/admin/get_referral_user_details?page=${page}&pageSize=${rowsPerPage}&searchTerm=${searchValue}`
      );
                      ...
                      recordsAfterPagingAndSorting().map((category, index) => (
                        <TableRow key={category._id}>
                          ...
                          <Tooltip title={NameText === category._id ? "Click to Hide" : "Click to View"} arrow>
                            ...
                            >
                              {get(category, "referral_code", "N/A")}
                          ...
                          <TableCell className={classes.textMiddle}>
                            <div>{get(category, "referrel_count.signUpCount", "N/A")}</div>
                          </TableCell>
                          <TableCell>
                            <div style={{ display: "flex", justifyContent: "center", alignItems: "center" }}>
                              <div style={{ cursor: "pointer" }}>
                                {" "}
                                <Tooltip title={<span className="TooltipCustomSize">{`${"View"}`}</span>} arrow>
                                  <VisibilityIcon
                                    onClick={() => {
                                      history.push(`/adminPanel/referList2/${category._id}`);
                                    }}
                                  />
                                </Tooltip>
                              </div>
